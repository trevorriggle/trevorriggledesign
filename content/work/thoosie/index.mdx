...
# No still plates declared. The copy specifies no images, and a slot whose
# `label` and `alt` cannot be written from the copy is a placeholder box that
# prints unfilled text on a live page. One slot — the poster, which the video
# slot requires — is the whole media surface until real alt text exists.
# Adding more is four lines each; MANIFEST.md picks them up on the next build.
images: []
