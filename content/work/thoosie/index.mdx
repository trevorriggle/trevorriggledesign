...
# `label` and `alt` I cannot write is a placeholder box that prints "TODO" on
# a live page. One slot — the poster, which the video slot requires — is the
# whole media surface until real alt text exists. Adding more is four lines
# each; MANIFEST.md picks them up on the next build.
images: []
